...
def change_secrets_in_one_setting(proxy, config, setting_name):
	...
	#print(config['802-11-wireless-security'])
        #print(config['802-11-wireless'])
	print ('---------config--------------')
	for key in config:
           print (key)
	print ('---------802-11-wireless--------------')
	for key in config['802-11-wireless']:
           #print (config['802-11-wireless'][key])
           print (key)
       ...
       #         new_secret = raw_input ("Enter new secret for %s in %s: " % (key, setting))
        ...
        #config['802-11-wireless']['ssid'] = "kalle"
        config['802-11-wireless'] = { 'ssid': dbus.ByteArray('kalle'), 'mode': 'infrastructure', 'security': '802-11-wireless-security', 'name': '802-11-wireless' } 
...
def change_secrets(service_name, con_path, config):
    ...
    #change_secrets_in_one_setting(con_proxy , config, '802-11-wireless')
    change_secrets_in_one_setting(con_proxy, config, '802-11-wireless-security')
...
if len(sys.argv) == 3:
    if sys.argv[2] == 'system':
        setting_service = "org.freedesktop.NetworkManager.Settings"
